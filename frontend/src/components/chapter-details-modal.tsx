...
export function ChapterDetailsModal({
  ...
  const handleToggleLesson = (lesson: Lesson) => {
    ...
    const newCompletedCount = lessons.filter((l) => (l.id === lesson.id ? !lesson.isCompleted : l.isCompleted)).length
    const updatedChapter: Chapter = {
      ...chapter,
      completedLessons: newCompletedCount,
      status:
        newCompletedCount === 0 ? "Not Started" : newCompletedCount === lessons.length ? "Completed" : "In Progress",
    }
    onUpdateChapter(updatedChapter)
  }
  ...
  const handleAddNewLesson = () => {
    ...
    onAddLesson({
      ...
      isCompleted: false,
      duration: newLesson.duration ? Number.parseInt(newLesson.duration) : undefined,
        ...
        <div className="space-y-6">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">Teacher: {teacher?.fullName || "Unknown"}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                  ...
                  {chapter.dueDate && (
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm">Due: {new Date(chapter.dueDate).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
              </>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {completedLessons}/{lessons.length} lessons completed ({Math.round(progressPercentage)}%)
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-orange-600">{pendingLessons}</div>
              </CardContent>
            </Card>
          </div>
            ...
            <TabsContent value="lessons" className="space-y-4">
              ...
              {showAddLesson && (
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex space-x-2">
                      <Button onClick={handleAddNewLesson} size="sm" disabled={!newLesson.title.trim()}>
                        Add Lesson
                      </Button>
                      <Button variant="outline" onClick={() => setShowAddLesson(false)} size="sm">
                        Cancel
                      </Button>
                    </div>
                  </CardContent>
              ...
              <div className="space-y-2">
                {lessons.map((lesson, index) => (
                  <Card key={lesson.id} className={lesson.isCompleted ? "bg-green-50 dark:bg-green-950/20" : ""}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4
                              className={`font-medium ${lesson.isCompleted ? "line-through text-muted-foreground" : ""}`}
                            >
              ...
              {lessons.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mb-4">Start by adding your first lesson to this chapter</p>
                  {canEdit && (
            ...
            <TabsContent value="progress" className="space-y-4">
              ...
              <div className="space-y-4">
                {lessons
                  ...
                  .map((lesson) => (
                    <div key={lesson.id} className="flex items-center space-x-4 p-3 bg-muted rounded-lg">
                      <CheckCircle className="h-5 w-5 text-green-600" />
                      <div className="flex-1">
                        <p className="font-medium">{lesson.title}</p>
                        <p className="text-sm text-muted-foreground">
                          Completed on {new Date(lesson.completedDate!).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
                {lessons.filter((l) => l.isCompleted).length === 0 && (
                  <p className="text-muted-foreground text-center py-8">No completed lessons yet</p>
                )}
              </div>
            </TabsContent>
